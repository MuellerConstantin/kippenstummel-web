...
export function InstallAppButton() {
  const t = useTranslations("JumbotronCta");
...
export function JumbotronCta() {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-8 p-4">
      <div>
        <Button onPress={() => router.push("/map")} className="p-3 font-bold">
          {t("gettingStarted")}
        </Button>
      </div>
      {(isInstallable || isIOSInstallable) && (
        <div className="flex flex-col items-center gap-8">
          ...
          <TooltipTrigger>
            <InstallAppButton />
            <Tooltip>{t("install.tooltip")}</Tooltip>
          </TooltipTrigger>
        </div>
      )}
    </div>
